...
3. Write `PR_DESCRIPTION.md` at that copy’s root. First line is the title: `[bug fix] <what this PR does>`. The `[bug fix]` token is only a reviewer flag that this is a bug-fix PR — it does not mean “describe the bug.” After the token, name the change (**67 characters or fewer** if possible; GitHub truncates a longer subject): `Omit unset seed on on-prem initialize like Cortex`, not `On-prem sends null seed` or `seed=None fails /initialize`. Then `## Summary` / `## Testing`. Keep it current for **this** group’s work only.
