...
if os.environ.get("DSS_CLIENT_REPO"):
    # Opt-in only. Hardcoding a checkout here used to silently win over the
    # caller's PYTHONPATH, which made it impossible to choose which dss-client
    # the run exercised.
...
def default_job_body() -> str:
    """``examples/training-small-hf.json`` from whichever dss-client is imported.

    Derived rather than hardcoded so this runs on any checkout. The example
    pins no image, which is what makes it usable for default-image checks.
    """
    import dss_client

    return str(Path(dss_client.__file__).resolve().parent.parent / "examples" / "training-small-hf.json")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if args.client_loss == "trl_grpo":
            ...
            # Resolve the checkout from the imported package rather than pinning
            # one clone, so the loss under test matches the transport under test.
            import arctic_platform

            repo = Path(arctic_platform.__file__).resolve().parent.parent
            sys.path.insert(0, str(repo))
            spec = importlib.util.spec_from_file_location(
                "_pr84_trl_loss",
                str(repo / "arctic_platform" / "integrations" / "trl" / "loss.py"),
            )
