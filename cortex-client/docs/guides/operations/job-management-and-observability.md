...
After you create a job, use the `dss-neutrino` CLI or `NeutrinoClient` to
inspect status, download logs, list checkpoints, and cancel or resume work.
...
The published page uses the upcoming `cortex-training` / `CortexTrainingClient`
names; on `main` the same operations are `dss-neutrino`, `neutrino-tui`, and
`NeutrinoClient`.
...
dss-neutrino download-log JOB_ID --output-dir ./logs
...
`NeutrinoClient` after you create a client once:
...
| Task | CLI | SDK |
| --- | --- | --- |
| Capacity | `dss-neutrino capacity` | `client.get_capacity()` |
| List jobs | `dss-neutrino list` / `list --status running` | `client.list_jobs()` / `client.list_jobs(status="running")` |
| Get job | `dss-neutrino get JOB_ID` | `client.get_job(job_id)` |
| Wait until running | `dss-neutrino wait JOB_ID` | `client.wait_for_job(job_id)` |
| List checkpoints | `dss-neutrino checkpoints JOB_ID` | `client.list_checkpoints(job_id)` |
| Download execution logs | `dss-neutrino download-log JOB_ID --output-dir DIR` | `client.fetch_execution_logs(job_id)` |
| Cancel | `dss-neutrino cancel JOB_ID` | `client.cancel_job(job_id)` |
| Load checkpoint | `dss-neutrino --job-id JOB_ID load CHECKPOINT_ID` | `client.load(job_id, checkpoint_id)` |
| Live tail | `neutrino-tui JOB_ID` | *(CLI / TUI only)* |
...
dss-neutrino list
dss-neutrino list --status running
dss-neutrino list --status failed
```
...
Run `neutrino-tui` without a job ID to open a job picker. Useful keys include
`/` to filter, `p` to pause, `s` to save visible logs, `r` to refresh, and `q`
to quit.
...
wrote under its local `log_path`. Live output is available via `neutrino-tui`
