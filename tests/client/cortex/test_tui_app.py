...
the optional ``textual`` extra is installed (``pip install 'arctic_platform[tui]'``).

One MagicMock stands in for both collaborators: it answers ``list``/``get`` like
`CortexJobs` and ``tail_logs`` like `CortexLogs`, and is passed as both."""
...
from arctic_platform.client.cortex.tui.app import CortexLogApp  # noqa: E402
from arctic_platform.client.cortex.tui.app import JobListScreen  # noqa: E402
from arctic_platform.client.cortex.tui.app import LogScreen  # noqa: E402
...
def _client():
    ...
    # Sources come from get().sub_jobs now (one per sub-job). RUNNING keeps
    # the live tail path; terminal-job behavior is covered by overriding get.
    c.get.return_value = {
        "status": "RUNNING",
        "sub_jobs": [
...
def _pair():
    """The (jobs, reader) pair `CortexLogApp` takes, both backed by one fake."""
    c = _client()
    return c, c
...
async def _run_with_job_id():
    app = CortexLogApp(*_pair(), "7", poll_interval=0.01)
...
async def _run_with_picker():
    app = CortexLogApp(*_pair(), poll_interval=0.01)  # no job_id -> picker
    async with app.run_test() as pilot:
        ok = await _wait(
...
async def _run_refresh_picker():
    app = CortexLogApp(*_pair(), poll_interval=0.01)
    async with app.run_test() as pilot:
        ok = await _wait(
            pilot, app, lambda: isinstance(app.screen, JobListScreen) and len(app.screen.query("#jobs ListItem")) == 2
        )
...
async def _run_offline_fallback():
    c = _client()
    c.get.side_effect = RuntimeError("network down")  # can't fetch sub-jobs
    app = CortexLogApp(c, c, "7", poll_interval=0.01)
    async with app.run_test() as pilot:
        ok = await _wait(
            pilot,
            app,
        ...
        assert ok, "cached sub-job not shown after get failure"
        await _settle(app, pilot)


def test_offline_shows_cached_sources(tmp_path, monkeypatch):
    # Seed a cached sub-job for job "7", then make get fail: the source list
    # must fall back to the cached sub-job so logs stay reachable.
    monkeypatch.setenv("CORTEX_LOG_CACHE_DIR", str(tmp_path))
...
async def _run_terminal_cache_only():
    c = _client()
    c.get.return_value = {  # terminal -> zone gone; metadata still lists sub-jobs
        "status": "FAILED",
...
async def _run_replay_lands_at_bottom():
    c = _client()
    c.get.return_value = {  # terminal → cache-only replay, no live poll
        "status": "FAILED",
        "sub_jobs": [{"sub_job_id": "7:training:0", "job_type": "training"}],
    }
    app = CortexLogApp(c, c, "7", poll_interval=0.01)
...
async def _run_export():
    ...
    async with app.run_test() as pilot:
        ...
        ok = await _wait(pilot, app, lambda: bool(glob.glob(os.path.expanduser("~/cortex-7-*.log"))))
        assert ok, "export file was not written"
        content = open(glob.glob(os.path.expanduser("~/cortex-7-*.log"))[0], encoding="utf-8").read()
