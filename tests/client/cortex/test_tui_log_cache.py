...
from arctic_platform.client.cortex.tui.log_cache import cache_root
from arctic_platform.client.cortex.tui.log_cache import cached_log_pages
from arctic_platform.client.cortex.tui.log_cache import cached_pages
from arctic_platform.client.cortex.tui.log_cache import safe_source_name
from arctic_platform.client.cortex.tui.log_cache import unsafe_source_name

# ─── cache dir resolution ────────────────────────────────────────────────


def test_cache_root_respects_xdg_cache_home(monkeypatch, tmp_path):
    monkeypatch.delenv("CORTEX_LOG_CACHE_DIR", raising=False)
    monkeypatch.setenv("XDG_CACHE_HOME", str(tmp_path))
    assert cache_root() == tmp_path / "cortex" / "logs"
...
def test_cache_root_falls_back_to_home_cache(monkeypatch, tmp_path):
    ...
    assert cache_root() == tmp_path / ".cache" / "cortex" / "logs"


def test_override_env_wins(monkeypatch, tmp_path):
    monkeypatch.setenv("CORTEX_LOG_CACHE_DIR", str(tmp_path / "override"))
