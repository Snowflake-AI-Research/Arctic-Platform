...
```bash
cp arctic_platform/client/recipes/config.json{.template,}
```

`arctic_platform/client/recipes/*.json` is gitignored, so a filled-in
`config.json` will not be committed. All commands below are run from the repo
root.
...
python -m arctic_platform.client.recipes.sft_loop config=arctic_platform/client/recipes/config.json

# RL — MATH, Qwen/Qwen3-8B, 4 train + 4 sample GPUs
# Set lora_rank explicitly for LoRA
python -m arctic_platform.client.recipes.rl_loop config=arctic_platform/client/recipes/config.json lora_rank=32
```
...
python -m arctic_platform.client.recipes.sft_loop config=arctic_platform/client/recipes/config.json wandb_project=arctic
python -m arctic_platform.client.recipes.rl_loop config=arctic_platform/client/recipes/config.json \
...
python -m arctic_platform.client.recipes.sft_loop config=arctic_platform/client/recipes/config.json lora_rank=0

# Smaller / larger LoRA
python -m arctic_platform.client.recipes.sft_loop config=arctic_platform/client/recipes/config.json lora_rank=16
python -m arctic_platform.client.recipes.sft_loop config=arctic_platform/client/recipes/config.json lora_rank=64
```
...
```bash
python -m arctic_platform.client.recipes.sft_loop config=arctic_platform/client/recipes/config.json max_length=4096
