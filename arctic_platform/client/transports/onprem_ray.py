...
class RayTransport(OnPremTransport):
    ...
    def _run_on_loop(self, coro):
        ...
        The loop runs forever in its own thread, so this works uniformly for every caller: sync callers
        (``SyncArcticRLClient``, destroy from a non-async thread), concurrent sync callers (TRL's main trainer
        thread and its background rollout worker both dispatch here), and async callers (``ArcticRLClient.shutdown``
        invoked under verl's ``asyncio.run``, whose own loop is a *different* loop). ``run_coroutine_threadsafe``
        schedules the coroutine on the loop thread and ``.result()`` blocks the caller until it completes -- no
        thread ever owns/drives the loop itself, so there is no "event loop is already running" race.
        """
