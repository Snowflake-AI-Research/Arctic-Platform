...
class CortexTransport(Transport):
    ...
    def _fwd_bwd(self, body: dict) -> dict:
        ...
        request_id = self._post_octet_request_chunks(
            path_suffix="forward-backward", operation="fwd-bwd", frame=payload, max_bytes=self._MAX_FWD_BWD_BYTES,
        )["request_id"]
    ...
    def _save_checkpoint(self, body: dict) -> dict:
        request_id = self._send(
            "POST", f"{self._prefix}/{self.job_id}/save", json={"checkpoint_type": "resumable"},
        ).json()["request_id"]
        return self._poll(request_id)
    ...
    def _generate(self, body: dict) -> dict:
        ...
        request_id = self._post_octet_request_chunks(
            path_suffix="generate", operation="generate", frame=frame, max_bytes=self._MAX_GENERATE_BYTES,
        )["request_id"]
    ...
    def _prefix(self) -> str:
        cfg = self.config
        return (
            f"{self.base_url}/api/v2/databases/{cfg.cortex_database}"
            f"/schemas/{cfg.cortex_schema}/{cfg.cortex_endpoint}"
        )
    ...
    def _build_sub_jobs(self) -> list[SubJobConfig]:
        ...
        if cfg.sampling_gpus > 0:
            subs.append(SubJobConfig.sampling_job(
                model_name=cfg.model_name, max_seq_len=cfg.max_seq_len, n_gpus=cfg.sampling_gpus,
                extra_sampling=cfg.vllm_config or {}, job_type=JobType.SAMPLING, dtype=cfg.dtype, seed=cfg.seed,
            ))
        if cfg.log_prob_gpus > 0:
            subs.append(SubJobConfig.sampling_job(
                model_name=cfg.model_name, max_seq_len=cfg.max_seq_len, n_gpus=cfg.log_prob_gpus,
                extra_sampling=cfg.vllm_config or {}, job_type=JobType.LOG_PROBABILITY, dtype=cfg.dtype, seed=cfg.seed,
            ))
        if cfg.training_gpus > 0:
            subs.append(SubJobConfig.training_job(
                model_name=cfg.model_name, optimizer=tc.get("optimizer", {"type": "adamw", "lr": 1e-5}),
                max_seq_len=cfg.max_seq_len, train_batch_size=tc.get("train_batch_size", 1),
                n_gpus=cfg.training_gpus, extra_training=tc, dtype=cfg.dtype, seed=cfg.seed,
            ))
        return subs
