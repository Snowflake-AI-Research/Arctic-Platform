...
# Cache dir resolution mirrors connection.login_path: an explicit override wins,
# else XDG_CACHE_HOME, else ~/.cache, namespaced under the app.
_OVERRIDE_ENV = "CORTEX_LOG_CACHE_DIR"
_XDG_CACHE_ENV = "XDG_CACHE_HOME"
_NAMESPACE = ("cortex", "logs")

# Backoff defaults, matching CortexLogs. Kept module-level so a test/mock reader
# need not carry them.
_POLL_BACKOFF_MULTIPLIER = 1.25
_POLL_MAX_INTERVAL = 6.0

_ERROR_LOG = os.path.expanduser("~/.cortex-log-errors.log")
...
def cache_root() -> Path:
    ...
    return base.joinpath(*_NAMESPACE)
...
class LogCache:
    ...
    def _rotate_if_needed(self, source_id: str) -> None:
        ...
        kept = list(deque(lines, maxlen=self._max_lines))
...
def cached_log_pages(
    ...
    *,
    ...
    """``cached_pages`` wired to ``CortexLogs.tail_logs`` for a sub-job's logs. The
    ...
    def fetch(cursor: Optional[str]) -> dict:
        ...
        return reader.tail_logs(job_id, **kw)
