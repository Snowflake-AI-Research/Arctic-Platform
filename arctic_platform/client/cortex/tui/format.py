...
"""Pure formatting helpers for the read-only log/event view.

Kept free of any ``textual`` import so they can be unit-tested without the
optional TUI dependency installed -- and so ``cortex logs --plain`` can render
the same lines without a terminal UI.
"""
