...
"""Read-only Textual log browser for Cortex jobs, launched by ``cortex logs``.

Flow: open → a list of jobs with their status → pick one → its log sources (one
per sub-job: zone server / job orchestration / Ray head) → live tail. Pass a job
id to jump straight to that job's logs and skip the picker.

Reads through `CortexJobs` (job list/detail) and `CortexLogs` (tail), so it shares
the CLI's connection and retry policy. Strictly read-only — it never mutates state.

Imports ``textual`` at module load, so it is only importable with the optional
``arctic_platform[tui]`` extra.
"""
...
from arctic_platform.client.cortex.tui.format import entry_at_level
from arctic_platform.client.cortex.tui.format import entry_matches
from arctic_platform.client.cortex.tui.format import format_job_config
from arctic_platform.client.cortex.tui.format import format_job_row
from arctic_platform.client.cortex.tui.format import format_log_entry
from arctic_platform.client.cortex.tui.format import is_active_status
from arctic_platform.client.cortex.tui.format import job_matches
from arctic_platform.client.cortex.tui.format import job_status_color
from arctic_platform.client.cortex.tui.format import sort_jobs
from arctic_platform.client.cortex.tui.format import source_label
from arctic_platform.client.cortex.tui.format import wrap_log_line
from arctic_platform.client.cortex.tui.log_cache import LogCache
from arctic_platform.client.cortex.tui.log_cache import cached_log_pages

_ERROR_LOG = "~/.cortex-log-errors.log"


class JobListScreen(Screen):
    """Pick a job, listed with its status."""
    ...
    def __init__(self, jobs, reader, *, sub_job_id=None, poll_interval=1.0):
        super().__init__()
        self._jobs = jobs
        self._reader = reader
    ...
    def on_mount(self) -> None:
        ...
        self._load_jobs()
        # Gentle auto-refresh so status changes appear without pressing r. Not named
        # _auto_refresh: that is a DOMNode attribute, and shadowing it breaks textual.
        self.set_interval(self._REFRESH_SECONDS, self._refresh_periodically)
    ...
    def _load_jobs(self) -> None:
        ...
        try:
            ...
            # The ignores below: textual types call_from_thread's coroutine argument
            # as Awaitable[Never], which no `async def ... -> None` can satisfy.
            error = f"listing jobs failed: {type(exc).__name__}: {exc}"
            self.app.call_from_thread(self._on_jobs, None, error)  # type: ignore[arg-type]
            return
        if worker.is_cancelled:
            return
        self.app.call_from_thread(self._on_jobs, jobs, None)  # type: ignore[arg-type]
...
class LogScreen(Screen):
    ...
    def __init__(self, jobs, reader, job_id, *, sub_job_id=None, poll_interval=1.0, job_status=None):
        super().__init__()
        self._jobs = jobs
        self._reader = reader
        self._job_id = job_id
    ...
    def action_save_log(self) -> None:
        ...
        path = os.path.expanduser(f"~/cortex-{self._job_id[:8]}-{safe}.log")
        try:
...
class CortexLogApp(App):
    ...
    def __init__(self, jobs, reader, job_id=None, *, sub_job_id=None, poll_interval=1.0):
        super().__init__()
        self._jobs = jobs
        self._reader = reader
        self._job_id = job_id
    ...
    def on_mount(self) -> None:
        if self._job_id:
            ...
            self.push_screen(
                JobListScreen(self._jobs, self._reader, sub_job_id=self._sub_job_id, poll_interval=self._poll_interval)
            )
