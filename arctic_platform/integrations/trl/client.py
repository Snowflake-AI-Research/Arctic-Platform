...
def _mimic_cortex() -> bool:
    return os.environ.get("TRL_MIMIC_CORTEX", "0") not in ("0", "false", "False", "")


def _mimic_cortex_current_token_frame() -> bool:
    """Send advantages/mask in Cortex TRL's current-token frame (experiment only)."""
    return os.environ.get("TRL_MIMIC_CORTEX_CURRENT_TOKEN_FRAME", "0") not in ("0", "false", "False", "")


def _post_processors() -> list[str]:
    # Cortex / PR 113: only compute_logprobs is registered under that name.
    if _mimic_cortex():
        return ["compute_logprobs"]
    return ["apply_temperature", "compute_entropy_and_logprobs"]
...
class ArcticTrainingClient:
    ...
    def __init__(
        ...
        *,
        ...
        self._mimic_cortex = _mimic_cortex()
        if self._mimic_cortex:
            # Cortex zone loss name on PR 113. Do not send processing.config.dp_size:
            # the server stamps meta["dp_size"]=len(workers) and resolve_global_loss_scale
            # raises on a client conflict.
            self.server_loss_fn = os.environ.get("TRL_LOSS_FN", "grpo")
            logger.warning(
                "TRL_MIMIC_CORTEX=1 server_loss_fn=%s posts=%s drop_pi_old=1 "
                "current_token_frame=%s",
                self.server_loss_fn,
                _post_processors(),
                _mimic_cortex_current_token_frame(),
            )
    ...
    def _processing(self, *, loss_fn: str | None, config: dict | None = None) -> dict:
        proc: dict[str, Any] = {"post": _post_processors(), "loss_fn": loss_fn}
        if config:
            proc["config"] = config
        return proc

    def _cortex_loss_config(self, ing: dict, *, batch_num_tokens: int, global_batch_size: int) -> dict | None:
        if not self._mimic_cortex:
            return None
        return {
            "eps_clip": float(ing["epsilon_low"]),
            "eps_clip_higher": float(ing["epsilon_high"]),
    ...
    @staticmethod
    def _shifted_logprobs_entropy(out: dict, seq_lens: torch.Tensor, device: torch.device):
        log_probs = _shift_for_trl(_repack_to_row(out["logprobs"], seq_lens)).to(device)
        ent = out.get("entropy")
        if ent is None:
            entropy = torch.zeros_like(log_probs)
        else:
            entropy = _shift_for_trl(_repack_to_row(ent, seq_lens)).to(device)
    ...
    def forward_backward(
        ...
        log_probs, entropy = self._shifted_logprobs_entropy(out, seq_lens, device)
            ...
            self.client.fwd_bwd(
                {
                    ...
                    "processing": self._processing(loss_fn=self.loss_fn),
                }
            )
    ...
    def _forward_backward_server_loss(
        ...
        if self._mimic_cortex and _mimic_cortex_current_token_frame():
            # Match Cortex TRL's current client: it unpacks the unshifted packed
            # advantages/completion_mask directly, while zone compute_logprobs is
            # predict-next. Reconstruct that frame from TRL's shifted closure cells.
            adv_bs = _to_current_token_bs(ing["advantages"], seq_lens)
            mask_bs = _to_current_token_bs(ing["completion_mask"].to(torch.float32), seq_lens)
        else:
        ...
        # Cortex TRL pops π_old; zone grpo uses logprobs.detach(). Keep mimic on-policy.
        if not self._mimic_cortex:
            batch_payload["old_log_probs"] = old_lp_bs
        ...
        response = self.client.fwd_bwd(
            {
                ...
                "processing": self._processing(
                    ...
                    config=self._cortex_loss_config(
                        ing,
                        batch_num_tokens=batch_num_tokens,
                        global_batch_size=int(old_lp_bs.shape[0]),
                    ),
                ),
            }
        )
    ...
    def _forward_backward_server_loss_zorro(
        ...
        if not self._mimic_cortex:
            batch_payload["old_log_probs"] = old_lp
        ...
        response = self.client.fwd_bwd(
            {
                ...
                "processing": self._processing(
                    ...
                    config=self._cortex_loss_config(
                        ing,
                        batch_num_tokens=batch_num_tokens,
                        global_batch_size=int(old_lp.shape[0]),
                    ),
                ),
            }
        )
...
def _to_current_token_bs(shifted: torch.Tensor, seq_lens: torch.Tensor) -> torch.Tensor:
    """TRL shifted frame -> padded current-token frame used by Cortex TRL.

    ``_to_server_bs`` produces the predict-next frame. Move it right one slot
    within each padded row, prepend zero, and never wrap across sequences.
    """
    predict_next = _to_server_bs(shifted, seq_lens)
    current = torch.zeros_like(predict_next)
    if current.shape[-1] > 1:
        current[..., 1:] = predict_next[..., :-1]
    return current
    ...
    response = client.fwd_no_grad(
        {
            ...
            "processing": {"post": _post_processors(), "loss_fn": None},
        }
    )
