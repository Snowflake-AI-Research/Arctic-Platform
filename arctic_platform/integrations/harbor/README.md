...
| Mode | Agent | Path | Status |
| --- | --- | --- | --- |
| Native (default) | `CortexRLAgent` (or any agent consuming `ArcticRLClient.reconnect_config`) | RL-shaped `/generate` | Converged E2E on Cortex; results below. |
| OpenAI-compat (`--sampling-api-base`) | Any Harbor `BaseAgent` speaking OpenAI-chat via LiteLLM | `/v1/chat/completions` / `/v1/completions` / `/v1/models` on the sampling sub-job, translated to `ReplicaPool.generate()` in-process | Router implemented on the on-prem server (`arctic_platform/openai_compat.py`). **Not yet exercised against a live vLLM.** Cortex path additionally needs sub-job URL routing (see [PLAN.md](./PLAN.md)). |
