...
"""verl-shaped GRPO loss executed server-side by ``arctic_platform.rl``.

Previously lived at ``arctic_platform/rl/processors/verl_grpo.py``; moved
here so all verl-integration code sits under one subpackage. The old
module remains as a backward-compat shim that re-exports every public
symbol from this file, so downstream callers doing
``from arctic_platform.rl.processors.verl_grpo import ...`` continue to
work.

Registered as loss function ``"verl_grpo"`` on
:mod:`arctic_platform.rl.processors.pipeline`'s ``LOSS_FNS`` registry
via the ``@register_loss_fn`` decorator below. The verl-side adapter
(:mod:`arctic_platform.integrations.verl.adapter`) sends
``processing={"loss_fn": "verl_grpo", ...}`` in its ``update_actor``
payload; the server picks the loss out of ``LOSS_FNS`` and runs it.
"""
...
def verl_grpo_loss(model_outputs: dict, batch: dict, meta: dict, config: dict, device: str):
    ...
    if verl_policy_config.use_kl_loss:
        ...
        kld = kl_penalty(
            ...
            kl_penalty=verl_policy_config.kl_loss_type,
        )
        kl_loss = agg_loss(
            loss_mat=kld,
            loss_mask=response_mask,
