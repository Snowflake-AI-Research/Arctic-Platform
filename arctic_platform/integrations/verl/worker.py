...
# Originally lived at ``verl/remote_backend/workers/arctic_rl/worker.py``
# in Snowflake-AI-Research/verl (fork of verl-project/verl, Apache-2.0).
# Moved here so the per-backend forwarder worker ships alongside its
# adapter, and is discovered by verl through the
# ``VERL_USE_EXTERNAL_MODULES`` plugin hook.
"""Arctic-RL forwarder worker.

Per-backend worker that drives the Arctic adapter
(:class:`arctic_platform.integrations.verl.adapter.ArcticRLClientWrapper`).
...
from verl.remote_backend.worker_utils import make_njt
from verl.remote_backend.worker_utils import normalize_backend_metrics
from verl.remote_backend.worker_utils import shift_nested_response_aligned_to_predict_next
...
# every process that loads this worker -- including Ray child procs,
# which do not inherit the driver's import side-effects. The driver-side
# registration is handled by verl's `VERL_USE_EXTERNAL_MODULES` hook
# importing `arctic_platform.integrations.verl.register`; the child-side
# registration relies on this eager import.
from arctic_platform.integrations.verl import adapter  # noqa: F401
