"""DeepEP-shaped Python Buffer over ``uccl.ep``.

Throughput dispatch/combine only. Adapted from uccl d56e349
``ep/deep_ep_wrapper/deep_ep/buffer.py``. The ``uccl`` wheel ships the C++
runtime (``uccl.ep.Buffer``); this module allocates tensors and passes pointers.
"""
...
from .bootstrap import EventOverlap
from .bootstrap import check_nvlink_connections
from .bootstrap import destroy_uccl
from .bootstrap import initialize_uccl
...
def _record_async_stream_ownership(
    tensors,
    comm_stream,
    *,
    async_finish: bool,
    ...
    if not async_finish:
        return
    _record_stream_safe(tensors, comm_stream)
    if allocate_on_comm_stream:
        _record_stream_safe(tensors, torch.cuda.current_stream())
...
class Buffer:
    ...
    def __init__(
        ...
        if hasattr(ep, "get_rdma_buffer"):
            ...
            scratch_dlpack, rdma_buffer_is_host_allocated = ep.get_rdma_buffer(num_rdma_bytes, device_index)
    ...
    def get_dispatch_layout(
        ...
        previous_tensors_to_record = (
            () if previous_event is None or previous_event.extra_tensors is None else previous_event.extra_tensors
        )
        ...
        _record_async_stream_ownership(
            tensors_to_record,
            self.get_comm_stream(),
    ...
    def dispatch(
        ...
        if handle is not None:
            ...
            alloc_ctx = torch.cuda.stream(self.get_comm_stream()) if allocate_on_comm_stream else nullcontext()
            with alloc_ctx:
                recv_x = torch.empty((alloc_recv_tokens, x.size(1)), device=x.device, dtype=x.dtype)
                recv_x_scales = (
                    None
                    if x_scales is None
                    else torch.empty(
                        ((alloc_recv_tokens,) if x_scales.dim() == 1 else (alloc_recv_tokens, num_scales)),
                        device=x.device,
            ...
            with alloc_ctx:
                recv_x = torch.empty((alloc_recv_tokens, x.size(1)), device=x.device, dtype=x.dtype)
                recv_src_idx = torch.empty((alloc_recv_tokens,), dtype=torch.int32, device=x.device)
    ...
    def combine(
        ...
        if topk_weights is not None and topk_weights.size(0) == 0:
            topk_weights = topk_weights.new_empty((1, num_topk), dtype=topk_weights.dtype, device=topk_weights.device)
        alloc_recv_tokens = max(num_recv_tokens, 1)
        alloc_ctx = torch.cuda.stream(self.get_comm_stream()) if allocate_on_comm_stream else nullcontext()
        with alloc_ctx:
            recv_x = torch.empty((alloc_recv_tokens, x.size(1)), device=x.device, dtype=x.dtype)
        ...
        # src_idx is listed separately from handle: the zero-token substitution above
        # rebinds it to a fresh tensor, and recording handle only covers the original.
        tensors_to_record = previous_tensors_to_record + (
            x,
            topk_weights,
            bias_0,
    ...
    def internode_combine(
        ...
        alloc_ctx = torch.cuda.stream(self.get_comm_stream()) if allocate_on_comm_stream else nullcontext()
        with alloc_ctx:
            combined_x = torch.empty((alloc_combined_tokens, x.size(1)), device=x.device, dtype=x.dtype)
        ...
        # src_meta is listed separately from handle: the zero-token substitution above
        # rebinds it to a fresh tensor, and recording handle only covers the original.
        tensors_to_record = previous_tensors_to_record + (
            x,
            topk_weights,
