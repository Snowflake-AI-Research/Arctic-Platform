...
class FusedOutputLinear(torch.nn.Linear):
    ...
    def forward(
        ...
        if labels is None:
            raise ValueError(
                "FusedOutputLinear requires labels: a next-token target cannot be reconstructed from the "
                "hidden states this layer receives, because the window it is given may end mid-row and "
                "the target for its last position is then a token it does not hold. Build labels on the "
...
class FusedCrossEntropyOutputLinear(torch.nn.Linear):
    ...
    def __init__(
        ...
        # Liger derives its chunk count from the token count and by default accumulates the weight gradient in the
        # weight's dtype, so in bf16 the same sum rounds differently depending on how the tokens are split across
        # calls: 3.7e-03 relative on a V=4096, H=512 head. accum_dtype keeps the accumulator, and each chunk's
...
def inject_prime_lm_head(
    ...
    assert (
        not hasattr(model.lm_head, "bias") or model.lm_head.bias is None
    ), f"model.lm_head.bias is not supported: {model.lm_head}\n{model}"

    logger = get_logger()

    # Gemma-style softcapping is not supported by this shared MoE integration.
    ...
    if final_logit_softcapping:
        ...
        if not fused_cross_entropy:
            raise NotImplementedError(
                "Gemma-style final_logit_softcapping is not supported by the shared MoE model "
                "integration; use fused_cross_entropy='liger'."
            )
    ...
    if fused_cross_entropy == "quack":
        ...
        logger.info(
            "Injecting fused cross-entropy LM head (Liger kernel)"
            + (" with float32 gradient accumulation" if fp32_lm_head else "")
        )
...
def _patch_model_forward(model: nn.Module) -> None:
    inner_forward_parameters = inspect.signature(model.model.forward).parameters
    inner_model_accepts_kwargs = any(
        parameter.kind is inspect.Parameter.VAR_KEYWORD for parameter in inner_forward_parameters.values()
    )
    inner_model_kwarg_names = frozenset(
        name
        for name, parameter in inner_forward_parameters.items()
        if parameter.kind
        in (
    ...
    def new_forward(
        ...
        **kwargs: object,
        ...
        if use_cache not in (None, False):
            raise ValueError("use_cache=True is not supported by the custom MoE LM head")
        ...
        inner_kwargs = (
            kwargs
            if inner_model_accepts_kwargs
            else {key: value for key, value in kwargs.items() if key in inner_model_kwarg_names}
        )
        outputs = self.model(
            input_ids=input_ids,
        ...
        # Slice hidden states for logits_to_keep, which is either a count of trailing columns or a 1D integer
        # tensor naming the kept columns. Action-mask positions index the full sequence, so they have to move
        # into the kept columns' space before the hidden states and labels are narrowed to it.
