...
try:
    ...
    Qwen3_5MoeVisionModel = None  # type: ignore

# Flash attention v3 (flash-attn-3 / flash_attn_interface, Hopper).
try:
    from flash_attn_interface import flash_attn_varlen_func as flash_attn_3_varlen_func
except ImportError:
    flash_attn_3_varlen_func = None  # type: ignore

# Flash attention v4 (flash-attn-4 / flash_attn.cute, Blackwell). Different varlen
# signature than v3 (qv is the 4th positional arg) -> called by keyword below.
try:
...
class Qwen3_5MoeGatedFlashAttention(Qwen3_5MoeGatedAttentionBase):
    """Gated softmax attention using Flash Attention varlen functions (v3 Hopper / v4 Blackwell)."""
    ...
    def __init__(self, config: Qwen3_5MoeGatedAttentionConfig, flash_attn_version: int = 3):
...
QWEN35MOE_ATTN_IMPL2CLASS = {
    ...
    # HF-style "flash_attention_4" -> FA4 (Blackwell) kernel; recognized by the
    # varlen/cu_seqlens plumbing (model_builder.py and the model forward) below.
...
class Qwen3_5MoeModel(Qwen3_5MoePreTrainedModel):
    ...
    def forward(
        ...
        if self.config._attn_implementation in ("flash_attention_2", "flash_attention_3", "flash_attention_4"):
            ...
            # an SP shard carrying global-offset positions still yields local cu_seqlens. Same as the
            # value-based form when each sample starts at position 0 (non-SP).
            is_segment_start = flat_position_ids == 0
...
class Qwen3_5MoeVLMModel(nn.Module):
    ...
    def __init__(self, config: PretrainedConfig):
        super().__init__()
        if Qwen3_5MoeVisionModel is None:
            raise ImportError(
                "Qwen3_5MoeVisionModel is not available in the installed transformers build; "
                "the VLM path of the carved-out qwen35 package cannot be used."
            )
        self.config = config
