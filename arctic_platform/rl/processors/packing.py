...
def pack_sequences(data: dict[str, Any]) -> dict[str, Any]:
    ...
    position_ids = torch.cat([torch.arange(int(len), device=attention_mask.device) for len in lens.tolist()])
    packed = {}
...
def unpack_sequences(
    ...
    cu_seqlens = F.pad(torch.cumsum(lens, dim=0, dtype=torch.int32), (1, 0), value=0)
    out = torch.full(
        (B, S, *packed_tensor.shape[1:]), pad_value, dtype=packed_tensor.dtype, device=packed_tensor.device
    )
    for i in range(B):
        start = cu_seqlens[i].item()
        end = cu_seqlens[i + 1].item()
