...
if True:  # deal with sys.path adjustment
    ...
    from arctic_platform.rl.zorro_train.qwen_attention_patcher import compare_debug_tensors
    from arctic_platform.rl.zorro_train.qwen_attention_patcher import debug_object
...
def compare_gradients(grad_list1, grad_list2, param_names=None, name1="baseline", name2="patched", threshold=0.05):
    ...
    """
    assert len(grad_list1) == len(grad_list2), "Gradient lists have different lengths"
    ...
    print(
        f"\n{status}: Global gradient norm relative diff = {abs(global_rel_diff.item())*100:.2f}% (threshold ="
        f" {threshold*100:.1f}%)"
    )
    return passed
...
def test_forward_no_grad(
    ...
    dedup_input_ids, dedup_position_ids, reconstruction_info = deduplicator.create_deduplicated_batch(
        input_ids=input_ids,
    ...
    print(
        f"Baseline logits norm: {output_baseline.logits.norm()}, Deduplicated logits norm:"
        f" {output_dedup.logits.norm()}, Deduplicated logits norm no patching:"
        f" {output_dedup_no_patching.logits.norm()}"
    )
