...
def test_perf(
    ...
    try:
        ...
        if hasattr(flash_attn_3, "flash_attn_func"):
            pr0("  Flash Attention 3 available: Yes")
        ...
        if torch.cuda.is_available():
            ...
            if cuda_arch < 90:
                pr0(
                    "  Note: Flash Attention 3 optimizations require sm90+ (H100). Will use FA2 kernels on"
                    f" sm{cuda_arch}."
                )
    except ImportError:
        pr0("  Warning: flash-attn-3 not installed. Install with: pip install flash-attn --no-build-isolation")
    ...
    if use_model_builtin_deduplicator:
        ...
        dedup_input_ids = torch.cat(
            list(micro_batches[i]["dedup_input_ids"].squeeze() for i in range(len(micro_batches)))
        ).unsqueeze(0)
        # pr(dedup_input_ids)
    ...
    if not use_model_builtin_deduplicator:
        ...
        if use_unpad:
            ...
            pr("  Unpad optimization: DISABLED")

        if use_unpad:
            position_ids = ZoRRoTrain._unpad_replicated_ids(position_ids, attention_mask)
            input_ids = ZoRRoTrain._unpad_replicated_ids(input_ids, attention_mask)
    ...
    if use_model_builtin_deduplicator:
        ...
        dedup_actor_model_once_patcher = Qwen3ModelOncePatcher(
            model,
            response_len=response_len,
            max_token_len=max_token_len,
            rollout_n=rollout_n,
    ...
    with profile(
        ...
        with record_function("dedup_forward"):
            ...
            if use_model_builtin_deduplicator:
                outputs = model(
                    input_ids=input_ids, position_ids=position_ids, attention_mask=attention_mask, use_cache=False
                )
    ...
    pr("-" * 80)
    pr(
        f"{'Forward:':20} {baseline_fwd_time:>11.3f}s {dedup_fwd_time:>11.3f}s"
        f" {baseline_fwd_time/dedup_fwd_time:>11.2f}x"
    )
    pr(
        f"{'Backward:':20} {baseline_bwd_time:>11.3f}s {dedup_bwd_time:>11.3f}s"
        f" {baseline_bwd_time/dedup_bwd_time:>11.2f}x"
    )
    pr(
        f"{'Total:':20} {baseline_total_time:>11.3f}s {dedup_total_time:>11.3f}s"
        f" {baseline_total_time/dedup_total_time:>11.2f}x"
    )
    pr("=" * 80)
