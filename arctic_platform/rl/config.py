...
class ArcticRLClientConfig(BaseModel):
    ...
    def _backend_from_env(cls, data: Any) -> Any:
        """Let ``ARCTIC_BACKEND=cortex`` override SkyRL's baked-in ``backend="local"``.

        SkyRL constructs this config itself and hard-codes ``"local"``, so
        treating that value as "unset" is the only way to point it at Cortex
        without patching SkyRL. An explicitly chosen non-default backend wins.
        """
    ...
    def _derive_host_port(self) -> "ArcticRLClientConfig":
        ...
        if self.backend == "cortex":
            return self  # no local server to address; connection lives on CortexConfig
