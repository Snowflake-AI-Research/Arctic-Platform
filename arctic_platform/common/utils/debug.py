...
class SynchronizedWallClockTimerSimple(SynchronizedWallClockTimer):
    ...
    $ grep --exclude-dir=.git -lIr "ENABLE_TIMERS = True" . | grep -v arctic_platform.common.utils.debug.py | xargs -r -n1 perl -pi -e 's|ENABLE_TIMERS = True|ENABLE_TIMERS = False|g'
    To flip on:
    $ grep --exclude-dir=.git -lIr "ENABLE_TIMERS = False" . | grep -v arctic_platform.common.utils.debug.py | xargs -r -n1 perl -pi -e 's|ENABLE_TIMERS = False|ENABLE_TIMERS = True|g'
