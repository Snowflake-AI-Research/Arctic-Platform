...
Invoked via ``trainer.override_entrypoint=arctic_rl.entrypoint`` on the
``skyrl.train.entrypoints.main_base`` command line. ARL-specific knobs live
under ``trainer.arctic_rl`` (see ``ArcticRLTrainerConfig`` in ``.config``).

This module is vendored into Arctic-Platform under
``recipes/rl/skyrl/_lib/arctic_rl/`` so the recipes are self-contained and do
not require a SkyRL repo checkout. See ``VENDOR.md`` next to this file for the
upstream source SHA and re-sync instructions.
"""
...
def main() -> None:
    ...
    # Forward the parent of this ``arctic_rl/`` package onto Ray workers'
    # PYTHONPATH so they can import ``arctic_rl.*`` when deserializing the
    # skyrl_entrypoint task. Two ``dirname`` calls take us from
    # ``<.../_lib/arctic_rl/entrypoint.py>`` up to ``<.../_lib/>`` — i.e. the
    # directory that has ``arctic_rl/`` as a child.
    _pkg_parent = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    _existing_pp = env_vars.get("PYTHONPATH") or os.environ.get("PYTHONPATH", "")
    env_vars["PYTHONPATH"] = _pkg_parent + (os.pathsep + _existing_pp if _existing_pp else "")
