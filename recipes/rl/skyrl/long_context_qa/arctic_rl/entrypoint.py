...
``skyrl_gym`` inside Ray workers, without touching the user's SkyRL checkout.

The trick is the ``@ray.remote`` ``skyrl_entrypoint`` task. Ray dispatches a
task by serializing a reference to its defining module (``<module>.<func>``);
the worker resolves it by *importing* that module. Upstream defines
``skyrl_entrypoint`` in ``integrations.arctic_rl.entrypoint``, so workers
import upstream's package — which registers ``bird``/``bird_sql`` but doesn't
know about ``long_context_qa``.

By re-defining ``skyrl_entrypoint`` here, workers import *this* module on
deserialization. That imports the ``arctic_rl`` package (which side-effect
registers ``long_context_qa`` via ``envs/__init__.py``) in the worker's
address space, so ``skyrl_gym.make("long_context_qa")`` works there too.
...
def skyrl_entrypoint(
    ...
    Defined here (not in upstream) so workers import THIS module — see module
    docstring for why.
    """
...
def main() -> None:
    ...
    - dispatches to the local ``skyrl_entrypoint`` ray task above (instead of
      upstream's), so workers re-import this package and re-register our envs.
    ...
    relative import chain (``arctic_rl`` -> ``arctic_rl.envs``) resolves in
    workers too.
    """
    # No explicit ``from . import envs``: ``__init__.py`` already does it.
    ...
    # arctic_rl.entrypoint`` -> trigger ``long_context_qa`` registration) onto
    # Ray workers' PYTHONPATH. The launcher sets SKYRL_HOME; we hard-error if
    # it's missing because the upstream config/trainer/generator imports above
    # already require it on the driver.
