...
def main():
    ...
    if args.json:
        out = {'result': res, 'runtime': rt, 'assumptions': {'per_token_factor': args.per_token_factor, 'ffn_fraction': args.ffn_fraction}}
    ...
    print(f"Per-token FLOPs = per_token_factor * P_eff = {args.per_token_factor} * {int(res['P_eff']):,} = {res['per_token_flops']:.3g} ({human(res['per_token_flops'])})")
